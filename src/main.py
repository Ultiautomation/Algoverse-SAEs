...
def main():
    ...
    tuned_model_name = "tomg-group-umd/zephyr-llama3-8b-sft-refusal-n-contrast-multiple-tokens"
    
    # Load tokenizer once
    tokenizer = get_tokenizer(tuned_model_name)

    # # Settings to intitiate IG on final output
    # SAVE_EVERY = 10
    ...
    for i, (_, row) in enumerate(sampled_df.iloc[start_index:].iterrows(), start=start_index):
        ...
        try:
            ...
            if (i + 1) % COND_SAVE_EVERY == 0:
                with open(LAYERWISE_COND_SAVE_PATH, "wb") as f:
    ...
    layerwise_cond_df.to_pickle(FINAL_COND_SAVE_PATH)
    print(f"\n✅ Layerwise conductance completed. Results saved to {FINAL_COND_SAVE_PATH}")
