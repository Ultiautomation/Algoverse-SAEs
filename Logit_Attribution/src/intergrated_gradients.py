...
# """
def run_ig(prompt, model, tokenizer, refusal_terms, baseline_strategy="pad", n_steps=5):
    ...
    embeddings = model.get_input_embeddings()(input_ids).detach().requires_grad_(True).to(device)

    # Get baseline (make sure get_baseline returns tensor on device)
    baseline = get_baseline(input_ids, model, tokenizer, strategy=baseline_strategy).to(device)

    print(f"embeddings shape: {embeddings.shape}, baseline shape: {baseline.shape}")
    print(f"embeddings device: {embeddings.device}, baseline device: {baseline.device}, attention_mask device: {attention_mask.device}")

    # Quick forward test to isolate issues
    try:
        with torch.no_grad():
    ...
    for phrase in refusal_terms:
        ...
        print(f"→ Phrase: {phrase} → Tokens: {target_tokens} → Token IDs: {token_ids}")
        ...
        for token_id in token_ids:
            print(f"Starting IG attribution for token_id: {token_id}")

            def forward_func(embeds):
                outputs = model(inputs_embeds=embeds, attention_mask=attention_mask)
                if outputs.logits.shape[1] == 0:
                    raise ValueError("Logits have zero length sequence dimension")
                return outputs.logits[:, -1, token_id]
            ...
            try:
                ...
                print(f"✅ Completed IG for token_id: {token_id}, delta: {delta}")
